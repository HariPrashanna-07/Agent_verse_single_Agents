...
def generate_insights(db: Session) -> list:
    ...
    for habit in habits:
        ...
        # Struggling / Goal too difficult — emit ONE insight per habit, not two.
        # STRUGGLING (< 40%) is more severe and subsumes GOAL_TOO_DIFFICULT (< 50%).
        if completion_rate < 40:
            insights.append({
                "type": "STRUGGLING",
                "habit_id": habit.id,
                "habit_name": habit.name,
                "message": f"{habit.name} — {completion_rate}% completion over the last 7 days. Consider temporarily reducing the target to build consistency.",
                "severity": "danger",
            })
        elif completion_rate < 50 and avg_target_pct < 70:
            insights.append({
                "type": "GOAL_TOO_DIFFICULT",
                "habit_id": habit.id,
                "habit_name": habit.name,
                "message": f"{habit.name} — {completion_rate}% completion this week. A slightly smaller target may improve consistency.",
                "severity": "warning",
            })
