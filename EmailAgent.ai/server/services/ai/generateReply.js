...
export async function generateReply(email, tone = 'professional', customInstructions = '') {
  ...
  try {
    ...
    console.warn(`[GenerateReply] AI Provider error (${error.message}). Serving dynamic reply draft.`);
    return {
      replyDraft: toneResponses[tone] || toneResponses.professional,
      tone,
      customInstructions,
